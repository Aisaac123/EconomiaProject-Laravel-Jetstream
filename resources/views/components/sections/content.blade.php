@props(['class' => 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-6 space-y-6',
        ...
        'isCollapsible' => true,
        'title' => null,
        'title_class' => 'text-2xl font-bold text-primary-700 dark:text-primary-300 mb-6',
])

<div class="space-y-2" x-data="{ open: {{ $collapsed ? 'false' : 'true' }} }">
    <div>
        <div class="{{ isset($title) ? 'flex justify-between items-center' : '' }} ">
            @if(isset( $title))
                <h2 {{ $attributes->merge(['class' => $title_class]) }}>{{ $title }}</h2>
            @endif
                @if($isCollapsible)
                    <div
                        type="button"
